...
void Text::Render(const char* text, float x, float y, float size)
{
    ...
    size = (size * 2.0) / 1920.0;

    std::vector<Vertex2D> vertices;
    for (unsigned int i = 0; i < length; i++)
    {
        glm::vec2 vertex_up_left = glm::vec2(x + i * size, y + size * 2.5);
        glm::vec2 vertex_up_right = glm::vec2(x + i * size + size, y + size * 2.5);
        glm::vec2 vertex_down_right = glm::vec2(x + i * size + size, y);
        glm::vec2 vertex_down_left = glm::vec2(x + i * size, y);

        char character = text[i];
        float uv_x = (character % 16) / 16.0f;
        float uv_y = 1.0 - ((character / 16)-2) / 16.0f;
